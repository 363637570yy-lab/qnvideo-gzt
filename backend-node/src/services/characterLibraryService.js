...
function applyLibraryItemToCharacter(db, log, characterId, libraryItemId) {
  ...
  const charRow = db
    ...
    .get(Number(characterId));
  if (!charRow) return { ok: false, error: 'character not found' };
  const drama = db.prepare('SELECT id, owner_user_id FROM dramas WHERE id = ? AND deleted_at IS NULL').get(charRow.drama_id);
  if (!drama) return { ok: false, error: 'unauthorized' };
...
// 加入本剧资源库（带 drama_id）
function addCharacterToLibrary(db, log, characterId, category, user = null) {
...
function addCharacterToMaterialLibrary(db, log, characterId, user = null) {
  const charRow = db.prepare('SELECT * FROM characters WHERE id = ? AND deleted_at IS NULL').get(Number(characterId));
  if (!charRow) return { ok: false, error: 'character not found' };
  if (!charRow.image_url && !charRow.local_path) return { ok: false, error: '角色还没有形象图片' };
  const now = new Date().toISOString();
 ...
 */
function batchGenerateCharacterImages(db, log, cfg, characterIds, modelName, style, aiConfigId, user, workflowPreset = null) {
  ...
  for (const characterId of ids) {
    ...
    setImmediate(async () => {
      try {
        const out = await generateCharacterFourViewImage(db, log, cfg, charId, modelName, style, aiConfigId, user, workflowPreset);
        if (!out.ok) {
...
async function generateCharacterFourViewImage(db, log, cfg, characterId, modelName, style, aiConfigId, user, workflowPreset = null) {
  const charRow = db.prepare(
  ...
  const userNeg = imageClient.resolveAssetUserNegativeForApi(
    modelName,
    workflowPresetService.mergeNegative(charRow.negative_prompt, workflowPreset)
  );
  const imageGen = imageClient.createAndGenerateImage(db, log, {
    drama_id: charRow.drama_id,
    character_id: charRow.id,
