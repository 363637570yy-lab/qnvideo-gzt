...
function generateImage(db, log) {
  ...
  return (req, res) => {
    ...
    const workflowPresetId = req.body?.workflow_preset_id || null;
    try {
...
function addToLibrary(db, log) {
  return (req, res) => {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) return response.badRequest(res, '无效的ID');
    const out = propLibraryService.addPropToLibrary(db, log, id, req.user);
    if (!out.ok) {
...
function addToMaterialLibrary(db, log) {
  return (req, res) => {
    const id = parseInt(req.params.id, 10);
    if (isNaN(id)) return response.badRequest(res, '无效的ID');
    const out = propLibraryService.addPropToMaterialLibrary(db, log, id, req.user);
    if (!out.ok) {
      if (out.error === 'prop not found') return response.notFound(res, '道具不存在');
      return response.badRequest(res, out.error);
    }
