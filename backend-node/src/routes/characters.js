  ...
  return {
    ...
    addToLibrary: (req, res) => {
      try {
        ...
        const out = characterLibraryService.addCharacterToLibrary(db, log, req.params.id, category, req.user);
        if (!out.ok) {
          if (out.error === 'character not found') return response.notFound(res, '角色不存在');
          return response.badRequest(res, out.error);
        }
    ...
    addToMaterialLibrary: (req, res) => {
      try {
        const out = characterLibraryService.addCharacterToMaterialLibrary(db, log, req.params.id, req.user);
        if (!out.ok) {
          if (out.error === 'character not found') return response.notFound(res, '角色不存在');
          return response.badRequest(res, out.error);
        }
