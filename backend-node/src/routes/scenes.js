...
function routes(db, log, cfg) {
  return {
    ...
    addToLibrary: (req, res) => {
      try {
        const out = sceneLibraryService.addSceneToLibrary(db, log, req.params.scene_id, req.user);
        if (!out.ok) {
          if (out.error === 'scene not found') return response.notFound(res, '场景不存在');
    ...
    addToMaterialLibrary: (req, res) => {
      try {
        const out = sceneLibraryService.addSceneToMaterialLibrary(db, log, req.params.scene_id, req.user);
        if (!out.ok) {
          if (out.error === 'scene not found') return response.notFound(res, '场景不存在');
          return response.badRequest(res, out.error);
        }
