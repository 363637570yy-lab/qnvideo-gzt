...
function createDrama(db, log) {
  return (req, res) => {
    ...
    try {
      ...
      if (req.user?.id) identityService.setDramaOwner(db, drama.id, req.user.id);
...
function getDramaStats(db, log) {
  return (req, res) => {
    try {
      const stats = dramaService.getDramaStats(db, req.user);
...
function importDrama(db, cfg, log) {
  return (req, res) => {
    try {
      ...
      if (req.user?.id && result?.drama_id) identityService.setDramaOwner(db, result.drama_id, req.user.id);
