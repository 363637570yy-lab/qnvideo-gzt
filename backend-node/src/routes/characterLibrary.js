...
function routes(db, cfg, log) {
  return {
    ...
    create: (req, res) => {
      try {
        const item = characterLibraryService.createLibraryItem(db, log, { ...(req.body || {}), user: req.user });
    ...
    get: (req, res) => {
      try {
        const item = characterLibraryService.getLibraryItem(db, req.params.id, req.user);
    ...
    update: (req, res) => {
      try {
        const item = characterLibraryService.updateLibraryItem(db, log, req.params.id, { ...(req.body || {}), user: req.user });
        if (!item) return response.notFound(res, '角色库项不存在');
    ...
    delete: (req, res) => {
      try {
        const ok = characterLibraryService.deleteLibraryItem(db, log, req.params.id, req.user);
